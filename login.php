...
<head>
    <meta charset="UTF-8">
    <title>Login || Web Design Studio</title>
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!empty($username) && !empty($password)) {
                ...
                if ($row) {
                    ...
                    $_SESSION["type"] = $row['type']; // Store account type

                    // Set cookies (optional)
                    setcookie("user_id", $row['u_id'], time() + (7 * 24 * 60 * 60), "/");
                    setcookie("username", $row['username'], time() + (7 * 24 * 60 * 60), "/");

                    // Update last login date and time
                    $current_date_time = date('Y-m-d H:i:s');
                    $updateStmt = $db->prepare("UPDATE users SET date=? WHERE u_id=?");
                    $updateStmt->bind_param("si", $current_date_time, $row['u_id']);
                    $updateStmt->execute();
                    ...
                    <div class="form">
                        ...
                        <form action="" method="post">
                            ...
                            <div style="margin-top: 10px;">
                                <a href="forgot_password.php" style="color: #5c4ac7; font-size: 14px;">Forgot Password?</a>
                            </div>
                            <div style="margin-top: 10px;">
                                <a href="forgot_username.php" style="color: #5c4ac7; font-size: 14px;">Forgot Username?</a>
                            </div>
